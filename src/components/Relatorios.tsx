...
export function Relatorios() {
  ...
  const handleGenerateReport = async () => {
    ...
    const mesReferencia = new Date(parseInt(year), parseInt(month) - 1, 15); // Dia 15 para evitar problemas de fuso

    if (reportType === 'folhaDePagamento') {
      dados_json.funcionarias = funcionarias
        .filter(f => f.status === 'Ativa')
        .map(f => {
          const faltasDoMes = faltas.filter(falta =>
          ...
          const descontos = faltasDoMes.length * (salarioBase / 30);
          const custoPassagens = (f.passagens_mensais || 0) * (f.valor_passagem || 0);
          const salarioFinal = salarioBase - descontos;
          return { nome: f.nome, salarioBase, custoPassagens, descontos, salarioFinal, faltas: faltasDoMes.length };
        });
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Selecione o tipo e o mês para gerar um novo relatório.</CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <Table>
            <TableHeader><TableRow><TableHead>Tipo</TableHead><TableHead>Mês</TableHead><TableHead>Gerado em</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader>
            <TableBody>
              {relatorios.length > 0 ? relatorios.map(r => (
                <TableRow key={r.id}>
                  <TableCell>{r.tipo_relatorio === 'folhaDePagamento' ? 'Folha de Pagamento' : 'Resumo de Faltas'}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button variant="outline" size="sm" className="mr-2" onClick={() => setSelectedReport(r)}><Eye className="h-4 w-4" /></Button>
                    <Button variant="outline" size="sm" className="mr-2" onClick={() => handleExportPDF(r)}><Download className="h-4 w-4" /></Button>
                    <Button variant="destructive" size="sm" onClick={() => deleteRelatorio(r.id)}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              )) : (
      ...
      <Dialog open={!!selectedReport} onOpenChange={() => setSelectedReport(null)}>
        <DialogContent className="max-w-4xl">
            ...
            {selectedReport?.tipo_relatorio === 'folhaDePagamento' && (
                <Table>
                    <TableHeader><TableRow><TableHead>Funcionária</TableHead><TableHead>Sal. Base</TableHead><TableHead>Custo Passagens</TableHead><TableHead>Descontos</TableHead><TableHead>Sal. Final</TableHead></TableRow></TableHeader>
                    <TableBody>
                        {selectedReport.dados_json.funcionarias.map((f:any, i:number) => <TableRow key={i}><TableCell>{f.nome}</TableCell><TableCell>R$ {f.salarioBase.toFixed(2)}</TableCell><TableCell>R$ {f.custoPassagens.toFixed(2)}</TableCell><TableCell>R$ {f.descontos.toFixed(2)}</TableCell><TableCell>R$ {f.salarioFinal.toFixed(2)}</TableCell></TableRow>)}
                        <TableRow className="font-bold bg-muted/50">
                            <TableCell>TOTAIS</TableCell>
                            <TableCell>R$ {selectedReport.dados_json.totais.salarioBase.toFixed(2)}</TableCell>
                            <TableCell>R$ {selectedReport.dados_json.totais.custoPassagens.toFixed(2)}</TableCell>
                            <TableCell>R$ {selectedReport.dados_json.totais.descontos.toFixed(2)}</TableCell>
                            <TableCell>R$ {selectedReport.dados_json.totais.salarioFinal.toFixed(2)}</TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
            )}
