...
const TransporteInfo = ({ condominio }: { condominio: Condominio }) => {
    const tipo = condominio.transporte_tipo;
    const detalhes = condominio.transporte_onibus_detalhes;

    if (tipo === 'veiculo_empresa') {
    ...
    if (tipo === 'onibus' && detalhes && detalhes.length > 0) {
        return (
            <div className="flex items-start gap-2 text-sm">
                <Bus className="w-4 h-4 text-muted-foreground mt-1" />
                <div className="flex flex-wrap gap-1">
                    {detalhes.map((onibus, i) => (
    ...
    return null; // Não mostra nada se for 'nenhum' ou não tiver detalhes
};
...
export function Condominios() {
    ...
    // Zera e popula os formulários ao abrir/fechar o dialog
    const openDialog = (condominio: Condominio | null = null) => {
        if (condominio) {
            // Modo de edição
            ...
            setLinhasDeOnibus(condominio.transporte_onibus_detalhes || []);
        } else {
            // Modo de adição
    ...
    const openDetailsDialog = (condominio: Condominio) => {
        setDetalhesCondominio(condominio);
        setEditingCondominio(null);
        setIsDialogOpen(true);
    }
    
    // Lógica para manipular a lista de ônibus no formulário
    const handleLinhaChange = (index: number, field: 'linha' | 'tipo', value: string) => {
        const novasLinhas = [...linhasDeOnibus];
        novasLinhas[index] = {...novasLinhas[index], [field]: value as any};
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingCondominio) {
                await updateCondominio(editingCondominio.id, dataToSave);
            } else {
                await createCondominio(dataToSave as Omit<Condominio, 'id'>);
            }
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex items-center justify-between">
                ...
                <Button onClick={() => openDialog()}><Plus className="w-4 h-4 mr-2" />Novo Condomínio</Button>
            </div>
                    ...
                    <Card key={condominio.id} className={`hover:shadow-lg transition-all duration-300 border-l-4 ${condominio.status === 'Ativo' ? 'border-l-yellow-400 bg-white' : 'border-l-red-300 bg-gray-100 text-muted-foreground'}`}>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex gap-2 pt-2">
                                <Button variant="outline" size="sm" onClick={() => openDialog(condominio)} className="flex-1"><Edit className="w-3 h-3 mr-1" />Editar</Button>
                                <Button variant="outline" size="sm" className="flex-1" onClick={() => openDetailsDialog(condominio)}><Eye className="w-3 h-3 mr-1" />Detalhes</Button>
                            </div>
                        </CardContent>
                    </Card>
                ))}
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="max-w-3xl flex flex-col h-auto md:h-[90vh]">
                    ...
                    {!detalhesCondominio && <DialogFooter className="pt-4"><Button type="button" variant="outline" onClick={closeDialog}>Cancelar</Button><Button type="submit" form="condo-form">{editingCondominio ? 'Salvar Alterações' : 'Cadastrar'}</Button></DialogFooter>}
                </DialogContent>
            </Dialog>
        </div>
